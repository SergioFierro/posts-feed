...
class CommentDaoTest : BaseDaoTest() {
  ...
  fun `Insert should store comment in DB`() = runBlocking {
    val mockComment = Comment(1, "name", "email", "body")
    dao.insert(mockComment)

    val commentsFromDB = dao.getAll()
    Assert.assertEquals(commentsFromDB[0], mockComment)
  }

  @Test
  @Throws(Exception::class)
  fun `Insert all should store comment in DB`() = runBlocking {
    val mockComment1 = Comment(1, "name", "email", "body")
    val mockComment2 = Comment(2, "name", "email", "body")
    val mockComment3 = Comment(3, "name", "email", "body")
    val mockList = listOf(mockComment1, mockComment2, mockComment3)
    dao.insert(mockList)

    val commentFromDB = dao.getAll()
    Assert.assertEquals(commentFromDB.toString(), mockList.toString())
    Assert.assertEquals(commentFromDB[0], mockComment1)
  }
  ...
  fun `Delete should delete comment in DB`() = runBlocking {
    ...
    Assert.assertFalse(dao.getAll().contains(mockComment1))
  }
}
